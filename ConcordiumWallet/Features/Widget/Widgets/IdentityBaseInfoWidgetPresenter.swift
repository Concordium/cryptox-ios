    ...
    init(identity: IdentityDataType) {
        ...
        case .confirmed:
            // Show expiry date for confirmed state
                if let seedIdentityObject = identity.seedIdentityObject {
                    bottomLabel = GeneralFormatter.formatISO8601Date(date: identity.seedIdentityObject?.attributeList.validTo ?? "")
                } else {
                    bottomLabel = GeneralFormatter.formatISO8601Date(date: identity.identityObject?.attributeList.validTo ?? "")
                }
            bottomIcon = "ok_icon"
