...
final class BakerAnountInputViewModel: ObservableObject {
    @Published var viewModel = StakeAmountInputViewModel()
    ...
    func loadData() {
        ...
        costRangeResult
            .onlySuccess()
            .map { $0.cost ?? .zero }
            .assignNoRetain(to: \.transferCost, on: viewModel)
            .store(in: &cancellables)
    ...
    private func handleParametersResult(_ result: Result<RemoteParameters, Error>) {
        switch result {
            case let .failure(error):
            viewModel.isContinueEnabled = false
//                self.view?.showErrorAlert(ErrorMapper.toViewError(error: error))
            case let .success(parameters):
                self.validator.minimumValue = parameters.minimumValue
                self.validator.maximumValue = parameters.maximumValue
