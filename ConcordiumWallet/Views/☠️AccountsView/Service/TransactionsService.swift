...
protocol TransactionsServiceProtocol {
    ...
    func performTransferUpdate(_ pTransfer: TransferDataType,
                         from account: AccountDataType,
...
extension TransactionsService {
    func performTransferUpdate(_ pTransfer: TransferDataType,
                               ...
                               contractAddress: ContractAddress1,
                               requestPasswordDelegate: RequestPasswordDelegate) -> AnyPublisher<TransferDataType, Error> {
        performPublicTransferUpdate(pTransfer, from: account, contractAddress: contractAddress, requestPasswordDelegate: requestPasswordDelegate)
    }
    
    private func performPublicTransferUpdate(_ pTransfer: TransferDataType,
                                       from account: AccountDataType,
                                       contractAddress: ContractAddress1,
                                       requestPasswordDelegate: RequestPasswordDelegate) -> AnyPublisher<(TransferDataType), Error> {
        ...
        transfer.expiry = expiry
        transfer.createdAt = Date()
        return getAccountNonce(for: transfer.from)
            .flatMap { (nonce: AccNonce) -> AnyPublisher<CreateTransferRequest, Error> in
                transfer.nonce = nonce.nonce
                return self.mobileWallet.createTransferUpdate(from: account,
                                                 to: transfer.toAddress,
                                                 contractAddress: contractAddress,
                                                 params: pTransfer.params,
                                                 receiveName: pTransfer.receiveName,
