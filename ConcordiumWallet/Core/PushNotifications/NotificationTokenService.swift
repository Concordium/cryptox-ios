...
final class NotificationTokenService {
    ...
    @MainActor
    func checkToken(from userInfo: [AnyHashable: Any]) async -> TokenResult? {
        guard let (contractName, contractIndex, contractSubindex, accountAddress) = extractContractDetails(from: userInfo) else {
            return nil
        }

        if let savedToken = defaultProvider.storageManager().getAccountSavedCIS2Tokens(accountAddress).first(where: {
